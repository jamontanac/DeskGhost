...
from deskghost.config import _load, _parse_hhmm, _require_bool, _require_int
...
def test_require_bool_true():
    assert _require_bool(True, "windows.send_keystrokes") is True


def test_require_bool_false():
    assert _require_bool(False, "windows.send_keystrokes") is False


def test_require_bool_rejects_string():
    with pytest.raises(ValueError, match="boolean"):
        _require_bool("true", "windows.send_keystrokes")  # type: ignore[arg-type]


def test_require_bool_rejects_int():
    with pytest.raises(ValueError, match="boolean"):
        _require_bool(1, "windows.send_keystrokes")  # type: ignore[arg-type]
...
def test_load_real_config_returns_all_keys():
    cfg = _load()
    expected_keys = {
        "IDLE_TIME_SECONDS", "MOVE_INTERVAL_SECONDS", "MOVE_DISTANCE_PIXELS",
        "WORK_START_TIME", "WORK_END_TIME", "WORK_DAYS",
        "LUNCH_START_TIME", "LUNCH_DURATION_MINUTES", "SEND_KEYSTROKES",
    }
...
def test_load_real_config_types():
    ...
    assert isinstance(cfg["MOVE_DISTANCE_PIXELS"],   int)
    ...
    assert isinstance(cfg["SEND_KEYSTROKES"],        bool)
...
def test_load_custom_path_overrides_values(tmp_path):
    ...
    assert cfg["MOVE_DISTANCE_PIXELS"]   == 5
    ...
    assert cfg["SEND_KEYSTROKES"]        is False
...
def test_load_invalid_send_keystrokes_type_raises(tmp_path):
    cfg_file = tmp_path / "config.yaml"
    cfg_file.write_text(textwrap.dedent("""\
        nudge:
          idle_time_seconds: 120
        ...
        windows:
          send_keystrokes: "yes"
    """))
    with pytest.raises(ValueError, match="windows.send_keystrokes"):
        _load(cfg_file)
...
def test_module_constants_match_loaded_config():
    ...
    assert config_module.MOVE_DISTANCE_PIXELS   == cfg["MOVE_DISTANCE_PIXELS"]
    ...
    assert config_module.SEND_KEYSTROKES        == cfg["SEND_KEYSTROKES"]
